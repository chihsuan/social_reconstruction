...
class FaceCV(object):
    ...
    def init_feature(self, detectorType):
    ...
    def faceMatch(self, minCount, face1, face2):
        ...
        for i in range(0, len(cropFaceList1)):
            ...
            #kp1 = self.detector.detect(cropFaceList1[i], None)
            #kp2 = self.detector.detect(cropFaceList2[i], None)
            #print kp1
            
            height, width = cropFaceList1[i].shape
           
            #kp1 = [cv2.KeyPoint(float(height/2), float(width/2), float(width))]
            #kp2 = [cv2.KeyPoint( float(height/2), float(width/2), float(width))]

            #kp1, desc1 = self.detector.compute(cropFaceList1[i], kp1)
            #kp2, desc2 = self.detector.compute(cropFaceList2[i], kp2)
    ...
    def match(self, minCount, face1, face2):
        ...
        if len(kp1) < 7 or len(kp2) < 7: #120 110
            #print 'img1 - %d features, img2 - %d features' % (len(kp1), len(kp2))
            return False
