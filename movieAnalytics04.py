            ...
            for j in range(startPoint, endPoint):
                ...
                for face1 in faceLists[keyword][key[i]]:
                    for face2 in faceLists[keyword][key[j]]:
                        if faceCV.faceMatch(minCount, face1, face2) and faceCV.faceMatch(minCount, face2, face1):
                            matchCount += 1  
    ...
    for word in bipartitleGraph: 
        ...
        if len(sortList) > 1:
            ...
            for j in range(1, len(sortList)):
                ...
                if not faceCV.faceMatch(8, character1[i], character2) and not faceCV.faceMatch(8, character1[2], character2):
                    print 'good match'
            ...
            for face1 in  mergeList[key[i]]:
                for face2 in mergeList[key[j]]:
                    if face1.keyword in word[key[j]] or face1.character == face2.character:        
                        ...
                        continue
                    if faceCV.faceMatch(7,face1, face2) and faceCV.faceMatch(7,face2, face1):
